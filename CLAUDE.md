...
- **Version A (bonus/roadmap, explicitly framed as forward-looking):** the identical engine, fed by sensor data instead of a human report, using a public dataset (AI4I 2020 Predictive Maintenance, or similar) as a stand-in. (Precisely: the shared schema, diagnosis-agent graph structure, and notification path are identical — the gather_context step needs a second, sensor-specific version, since it currently computes ERP-shaped signals like days-overdue and product/location match counts. This is a small, additive piece of work, not a rewrite — worth stating accurately rather than implying zero new code, per this project's honesty-about-what's-simulated-vs-real principle.) Pitch this as "ready to retrain on real sensor parameters the moment Tesla adds sensors" — not as solving a problem that exists today.
